...
- Feature(store): Add optional `timeout` to `withNgxsPendingTasks` [#2492](https://github.com/ngxs/store/pull/2492)
...
- Feature(storage-plugin): Add `withNgxsStorageSync` for cross-tab state sync [#2475](https://github.com/ngxs/store/pull/2475)
- Feature(storage-plugin): Report deserialize/serialize/quota failures via `ErrorHandler` [#2476](https://github.com/ngxs/store/pull/2476) [#2477](https://github.com/ngxs/store/pull/2477)
- Feature(router-plugin): Add derived selectors for query params, params, data, and title [#2471](https://github.com/ngxs/store/pull/2471)
...
- Fix(store): Avoid `NG0205`/`ObjectUnsubscribedError` during `TestBed` teardown [#2480](https://github.com/ngxs/store/pull/2480)
- Fix(store): Warn when a state uses `@Service()` without `autoProvided: false` [#2486](https://github.com/ngxs/store/pull/2486)
...
- Fix(storage-plugin): Shallow-merge instead of replacing state on cross-tab sync with `keys: '*'` [#2497](https://github.com/ngxs/store/pull/2497)
...
- Performance(store): Only attach `catchError` in `Store.select` when `suppressErrors` is enabled [#2472](https://github.com/ngxs/store/pull/2472)
- Performance(store): Skip the plugin middleware machinery when no plugins are registered [#2498](https://github.com/ngxs/store/pull/2498)
- Performance(store): Cache the state-bound selector function per selector [#2499](https://github.com/ngxs/store/pull/2499)
- Performance(store): Collapse the per-action dispatch-result pipeline [#2500](https://github.com/ngxs/store/pull/2500)
- Performance(store): Fast path for single-argument selectors [#2501](https://github.com/ngxs/store/pull/2501)
- Performance(store): Cache the resolved action type per action class [#2502](https://github.com/ngxs/store/pull/2502)
- Performance(store): Hoist dispatch RxJS operator config and drop the fast-path replay layer [#2503](https://github.com/ngxs/store/pull/2503)
- Refactor(store): Seed the state stream from the initial state token in its own constructor [#2473](https://github.com/ngxs/store/pull/2473)
- Refactor(router-plugin): Move derived selectors to tree-shakable exports, add named-outlet variants [#2474](https://github.com/ngxs/store/pull/2474)
...
- Performance(storage-plugin): Replace closure-based action matcher with direct type comparison [#2369](https://github.com/ngxs/store/pull/2369)
