...
import { iif, compose, patch } from '../../operators/src';
...
describe('[TEST]: the compose State Operator', () => {
  it('should return the correct null or undefined type', () => {
    ...
    compose(
      () => null,
      () => undefined
    ); // $ExpectType (existing: null | undefined) => null | undefined
  });

  it('should return the correct number type', () => {
    compose(() => 10); // $ExpectType (existing: number) => number
    // TS3.4 compose((x) => 10); // $ExpectType (existing: Readonly<{}>) => Readonly<{}>
    ...
    // TS3.4 compose((x) => 10, (x) => 20); // $ExpectType (existing: Readonly<{}>) => Readonly<{}>
    compose<number>(
      x => 10,
      x => 20
    ); // $ExpectType (existing: number) => number
    ...
    compose<number | string>(
      x => 10,
      x => 'abc'
    ); // $ExpectType (existing: string | number) => string | number
    compose<number | boolean>(
      x => 10,
      x => true
    ); // $ExpectType (existing: number | boolean) => number | boolean
    compose<number | boolean>(
      x => 10,
      x => false
    ); // $ExpectType (existing: number | boolean) => number | boolean
    // TS3.4 compose<number | number[]>((x) => 10, (x) => [123]); // $ExpectType (existing: number | number[]) => number | number[]
    compose<number | { val: number }>(
      x => 10,
      x => ({ val: 123 })
    ); // $ExpectType (existing: number | Readonly<{ val: number; }>) => number | Readonly<{ val: number; }>
  });

  it('should return the correct string type', () => {
    compose(() => 'abc'); // $ExpectType (existing: string) => string
    // TS3.4 compose((x) => 'abc'); // $ExpectType (existing: Readonly<{}>) => Readonly<{}>
    ...
    // TS3.4 compose((x) => 'abc', (x) => 'bcd'); // $ExpectType (existing: Readonly<{}>) => Readonly<{}>
    compose<string>(
      x => 'abc',
      x => 'bcd'
    ); // $ExpectType (existing: string) => string
    ...
    compose<string | boolean>(
      x => 'abc',
      x => true
    ); // $ExpectType (existing: string | boolean) => string | boolean
    compose<string | boolean>(
      x => 'abc',
      x => false
    ); // $ExpectType (existing: string | boolean) => string | boolean
    // TS3.4 compose<string | string[]>((x) => 'abc', (x) => ['abc']); // $ExpectType (existing: string | string[]) => string | string[]
    compose<string | { val: string }>(
      x => 'abc',
      x => ({ val: 'abc' })
    ); // $ExpectType (existing: string | Readonly<{ val: string; }>) => string | Readonly<{ val: string; }>
  });

  it('should return the correct implied boolean type', () => {
    compose(() => true); // $ExpectType (existing: boolean) => boolean
    // TS3.4 compose((x) => true); // $ExpectType (existing: Readonly<{}>) => Readonly<{}>
    ...
    // TS3.4 ((x) => true, (x) => false); // $ExpectType (existing: Readonly<{}>) => Readonly<{}>
    compose<boolean>(
      x => true,
      x => false
    ); // $ExpectType (existing: boolean) => boolean
    ...
    compose<boolean | number>(
      x => true,
      x => 123
    ); // $ExpectType (existing: number | boolean) => number | boolean
    // TS3.4 compose<boolean | boolean[]>((x) => true, (x) => [true]); // $ExpectType (existing: boolean | boolean[]) => boolean | boolean[]
    compose<boolean | { val: boolean }>(
      x => true,
      x => ({ val: true })
    ); // $ExpectType (existing: boolean | Readonly<{ val: boolean; }>) => boolean | Readonly<{ val: boolean; }>
  });
  ...
  it('should return the correct object type', () => {
    ...
    // TS3.4 compose((x) => ({ val: 10 })); // $ExpectType (existing: Readonly<{}>) => Readonly<{}>
    ...
    compose(
      () => ({ val: 10 }),
      () => ({ val: 20 })
    ); // $ExpectType (existing: Readonly<{ val: number; }>) => Readonly<{ val: number; }>
    compose(
      () => ({ val: 10 }),
      x => ({ val: 20 })
    ); // $ExpectType (existing: Readonly<{ val: number; }>) => Readonly<{ val: number; }>
    ...
    // TS3.4 compose((x) => ({ val: 10 }), (x) => ({ val: 20 })); // $ExpectType (existing: Readonly<{}>) => Readonly<{}>
    compose<{ val: number }>(
      x => ({ val: 10 }),
      x => ({ val: 20 })
    ); // $ExpectType (existing: Readonly<{ val: number; }>) => Readonly<{ val: number; }>
    ...
    compose(
      () => ({ val: 10 }),
      () => null
    ); // $ExpectType (existing: Readonly<{ val: number; }> | null) => Readonly<{ val: number; }> | null
    compose(
      () => ({ val: 10 }),
      () => undefined
    ); // $ExpectType (existing: Readonly<{ val: number; }> | undefined) => Readonly<{ val: number; }> | undefined
    compose<{ val: number } | null>(
      x => ({ val: 10 }),
      x => null
    ); // $ExpectType (existing: Readonly<{ val: number; }> | null) => Readonly<{ val: number; }> | null
    compose<{ val: number } | undefined>(
      x => ({ val: 10 }),
      x => undefined
    ); // $ExpectType (existing: Readonly<{ val: number; }> | undefined) => Readonly<{ val: number; }> | undefined
    compose<{ val: number } | number>(
      x => ({ val: 10 }),
      x => 123
    ); // $ExpectType (existing: number | Readonly<{ val: number; }>) => number | Readonly<{ val: number; }>
    compose<{ val: number } | string>(
      x => ({ val: 10 }),
      x => 'abc'
    ); // $ExpectType (existing: string | Readonly<{ val: number; }>) => string | Readonly<{ val: number; }>
    compose<{ val: number } | boolean>(
      x => ({ val: 10 }),
      x => true
    ); // $ExpectType (existing: boolean | Readonly<{ val: number; }>) => boolean | Readonly<{ val: number; }>
    compose<{ val: number } | boolean>(
      x => ({ val: 10 }),
      x => false
    ); // $ExpectType (existing: boolean | Readonly<{ val: number; }>) => boolean | Readonly<{ val: number; }>
    // TS3.4 compose<{ val: number; } | Array<{ val: number; }>>((x) => ({ val: 10 }), (x) => [{ val: 123 }]); // $ExpectType (existing: { val: number; }[] | Readonly<{ val: number; }>) => { val: number; }[] | Readonly<{ val: number; }>
    compose<{ val: number } | { val: { val: number } }>(
      x => ({ val: 10 }),
      x => ({ val: 123 })
    ); // $ExpectType (existing: Readonly<{ val: number; }> | Readonly<{ val: { val: number; }; }>) => Readonly<{ val: number; }> | Readonly<{ val: { val: number; }; }>
  });
});
